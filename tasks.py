...
def start(c):
    ...
    start_workers(c)
    with Builder(c):
        run_in_docker_or_locally_for_dinghy(c, 'yarn run build', no_deps=True)
...
def generate_certificates(c):
    ...
    with Builder(c):
        ...
        c.run('mkcert -cert-file docker/services/router/etc/ssl/certs/cert.pem -key-file docker/services/router/etc/ssl/certs/key.pem %s "*.%s"' % (c.root_domain, c.root_domain))
...
def docker_compose(c, command_name, bare_run=False):
    ...
    cmd = 'docker-compose -p %s %s %s' % (
        c.project_name,
        ' '.join('-f "' + c.root_dir + '/docker/' + file + '"' for file in c.docker_compose_files),
        command_name
    )
