...
env.project_name = 'forecast_tools'
...
env.project_hostnames = 'local.forecast.jolicode.com'
...
def migrate():
    ...
    # docker_compose_run('bin/console doctrine:migrations:migrate -n', no_deps=True)
...
def docker_compose(command_name):
    ...
    with shell_env(**localEnv):
        local('docker-compose -p %s %s %s' % (
            env.project_name,
            ' '.join('-f ' + env.root_dir + '/docker/' + file for file in env.compose_files),
            command_name
        ))
